...
<script>
    ...
    function Usermailer(user_email, toilet_id, sector, button) {
    ...
    function viewComplaint(user_email, toilet_id, sector, row, inspector_mail, message) {
        var modal = '<div class="modal fade" id="complaintModal" tabindex="-1" role="dialog" aria-labelledby="complaintModalLabel" aria-hidden="true">' +
            ...
            '<button type="button" id="respondUser" class="btn btn-primary" onclick="Usermailer(\'' + user_email + '\', \'' + toilet_id + '\', \'' + sector + '\', this)" disabled>Respond User</button>' +
            '</div>' +
            '</div>' +
            '</div>' +
            '</div>';
